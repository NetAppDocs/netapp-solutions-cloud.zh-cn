...
[role="lead"]
使用快照和备份在 OpenShift Virtualization 中保护虚拟机。此过程包括使用 ONTAP S3 对象存储创建 AppVault，配置 Trident Protect 以捕获虚拟机数据，包括 Kubernetes 资源对象、持久卷和内部映像，并在必要时恢复数据。
...
Trident Protect 支持 OpenShift 集群上的应用程序和 VM 的快照、备份、恢复和灾难恢复。对于 OpenShift Virtualization VM，可以使用 Trident Protect 保护的数据包括与 VM 关联的 Kubernetes 资源对象、持久卷和内部映像。
...
[%collapsible%open]
====
在为应用程序或 VM 创建快照和备份之前，必须在 Trident Protect 中配置对象存储以存储快照和备份。这是使用 bucket CR 完成的。只有管理员可以创建 bucket CR 并对其进行配置。bucket CR 在 Trident Protect 中称为 AppVault。AppVault 对象是存储桶的声明性 Kubernetes 工作流表示。AppVault CR 包含在保护操作中使用存储桶所需的配置，例如备份、快照、还原操作和 SnapMirror 复制。
...
在示例中，demo 命名空间有一个 VM，并且在创建应用程序时包含命名空间的所有资源。
